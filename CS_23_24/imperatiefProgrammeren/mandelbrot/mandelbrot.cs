...
// Colors
// The following colors are used in the color list for 'regenboogLijst' and were created using the color palette sourced from https://colorswall.com/palette/263884
...
// The following colors are used in the color list 'koeleLijst' and were created using the color palette sourced from https://www.freepik.com/premium-vector/blue-purple-color-palette-with-hex_33938791.htm
...
// The following colors are used in the color list 'vurigeLijst' and were created using the color palette sourced from https://colorpalette.org/red-orange-yellow-color-palette/
...
// The following colors are used in the color list 'grijzeLijst' and were created using the color palette sourced from https://design.gs.com/foundation/color-system/creating-the-system
...
// The following colors are used in the color list 'zomerLijst' and were created using the color palette sourced from https://www.color-hex.com/color-palette/12423 and https://icolorpalette.com/download/palette/225882_color_palette.jpg
...
// The following colors are used in the color list 'groeneLijst' and were created using a color palette generated by https://coolors.co/generate
...
// The following colors are used in the color list 'winterLijst' and were created using the color palette sourced from https://www.schemecolor.com/pastels-for-winter.php
Color donkerBlauw = Color.FromArgb(128, 160, 214);
Color ijsBlauw = Color.FromArgb(194, 246, 255);
Color beige = Color.FromArgb(255, 248, 247);
Color gebrokenWit = Color.FromArgb(237, 251, 255);
Color felBlauw = Color.FromArgb(161, 231, 255);
Color viezeSneeuw = Color.FromArgb(195, 187, 199);

// The following colors are used in the color list 'lenteLijst' and were created using the color palette sourced from https://nl.pinterest.com/pin/pastels-tutorials-guide-for-editing-simple-pastelishdoodled-covers--15340454971088901/
...
// The following colors are used in the color list 'fractalLijst' and were created by StackOverflow user 'q9f' in this thread: https://stackoverflow.com/questions/16500656/which-color-gradient-is-used-to-color-mandelbrot-in-wikipedia
Color brown0 = Color.FromArgb(204, 128, 0);
Color brown1 = Color.FromArgb(153, 87, 0);
Color brown2 = Color.FromArgb(106, 52, 3);
Color brown3 = Color.FromArgb(66, 30, 15);
Color darkViolet = Color.FromArgb(25, 7, 26);
Color darkestBlue = Color.FromArgb(9, 1, 47);
Color blue5 =  Color.FromArgb(4, 4, 73);
Color blue4 = Color.FromArgb(0, 7, 100);
Color blue3 = Color.FromArgb(12, 44, 138);
Color blue2 = Color.FromArgb(24, 82, 177);
Color blue1 = Color.FromArgb(57, 125, 209);
Color blue0 = Color.FromArgb(134, 181, 229);
Color lightestBlue = Color.FromArgb(211, 236, 248);
Color lightestYellow = Color.FromArgb(241, 233, 191);
Color lightYellow = Color.FromArgb(248, 201, 95);
Color dirtyYellow = Color.FromArgb(255, 170, 0);

// Declaring color lists
...
Color[] zomerLijst = new Color[] { boterGeel, lichtGroen, luchtBlauw, bosGroen, grasGroen, boterGeel2, lichtGroen2, luchtBlauw2, bosGroen2, grasGroen2 };
...
Color[] fractalLijst = new Color[] { brown0, brown1, brown2, brown3, darkViolet, darkestBlue, blue5, blue4, blue3, blue2, blue1, blue0, lightestBlue, lightestYellow, lightYellow, dirtyYellow };
...
// Instantiating and settings of labels
...
// Instantiating and settings of TextBoxes
...
// Instantiating and settings of buttons
...
// Instantiating and settings of trackbars
...
// 'Color' context menu and toolstrips
...
// 'Preset' context menu and toolstrips
...
presetToolStrip13.Text = "Elektriciteit";
...
// 'Best preset' context menu and toolstrips
...
bestToolStrip1.Text = "Best preset 1 - Gloeiende infectie";
bestToolStrip2.Text = "Best preset 2 - Hypnose";
bestToolStrip3.Text = "Best preset 3 - Seahorse valley";
bestToolStrip4.Text = "Best preset 4 - Kleurrijk vlokje";
bestToolStrip5.Text = "Best preset 5 - Spanning";
...
// Moving around using mouseclick
...
// Getting the user requested amount of movement from the TextBox,
// making sure the user input is in range and is not NaN or null
...
{
    // Check if 'reset' button or any 'best toolstrip' button is pressed
    if(resetButtonClicked == false)
    {
        presetButton.Text = $"{lastPresetToolStrip}";
    }
    if(bestPresetClicked == false)
    {
        lastBestToolStrip.Checked = false;
    }

    // Resetting both buttons to their unpressed state
    resetButtonClicked = false;
    bestPresetClicked = false;

    // Only perform RGB calculation when the values are used
    // Skipping will marginally improve computing time
    if (kleurenSelectie == "RGB sliders" || kleurenSelectie == "Donker - Licht")
    {
        rekenenRGB();
    }

    // Calculating the escape time for each pixel, starting at row 0, column 0.
    // Moving up one column after each pixel is calculated, until column == mandelGrootte,
    // then adding 1 to row. End if row == mandelGrootte.
    for (int row = 0; row < mandelGrootte; row++)
    {
        for (int column = 0; column < mandelGrootte; column++)
        {
            ...
            // Finding the coordinates of the current pixel
            ...
            // Color generation with respect to the selected generation type
...
// Method to reset all user selected options to default values
...
// Calculating the RGB value, making sure the user input is not NaN or null and is within bounds
void rekenenRGB()
{
    ...
    // Setting user input to either min or max value whenever user generated input is lower or higher than range
    if (ld > 255)
    {
        ld = 255;
...
// Methods to change UI visibility
// These exist purely to not bloat the look of the methods that call them
...
// Methods to show the ToolStripMenus on button press
...
// Methods to change the x, y, scale and max iterations to preset values
...
void presetToolStrip8_Click(object o, EventArgs e)
{
    ...
    middenyBox.Text = "0";
    schaalBox.Text = "2,235446990500142E-09";
    aantalBox.Text = "100";
...
{
    middenxBox.Text = "-1,2658955298066659";
    middenyBox.Text = "0,41407914945718394";
    schaalBox.Text = "7,137082610617687E-13";
    aantalBox.Text = "250";
...
// Methods to update UI to display the color generation that's selected
...
// Methods to display the best coordinate and color preset combinations
void bestToolStrip1_Click(object o, EventArgs e)
{
    kleurenToolStrip3_Click(null, null);
    ...
    bestPresetClicked = true;
    presetToolStrip19_Click(null, null);
}
...
{
    kleurenToolStrip1_Click(null, null);
    presetToolStrip13_Click(null, null);
...
// Methods to display color presets on button press
...
// Methods to update corresponding TextBoxes whenever a scrollbar value gets updated
...
// Showing ToolStripMenu on button press
presetButton.Click += showPresets;
kleurenGenButton.Click += showKleuren;
bestPresetButton.Click += showBestPresets;

// Calling the function to change the coordinate preset to whichever linked ToolStripMenuItem is clicked
...
// Calling the function to change the color generation type to whichever linked ToolStripMenuItem is clicked
...
// Calling the function to change the color preset to whichever linked ToolStripMenuItem is clicked
...
// Calling the function to display the best coordinate and color preset combinations
...
// Calling the function to update the corresponding TextBox whenever a scrollbar value is updated
